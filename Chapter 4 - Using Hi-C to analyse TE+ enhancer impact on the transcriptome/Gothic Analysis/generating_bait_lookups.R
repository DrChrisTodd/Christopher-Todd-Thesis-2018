...
rna.seq.file = '~/todd/Data/RNA-seq/TSC_RNA_FPKM.txt.gz'
...
##generate a bed list of promoter regions for TSC_RNA 
prom.start=c()
prom.end=c()
...
write.table(e.genes.look[,c(4,8)],"./Data/PCHiC/bait_ENSMBL_genes_look.txt",sep="\t",col.names = F,row.names = F,quote = F)
write.table(e.ncrna.look[,c(4,8)],"./Data/PCHiC/bait_ENSMBL_ncrna_look.txt",sep="\t",col.names = F,row.names = F,quote = F)
